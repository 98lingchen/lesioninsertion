"""Diffusion pipelines for conditional medical image generation.

This module contains latent-space and image-space diffusion pipeline variants
used for lesion generation and inpainting. It is designed to be imported by
training and inference scripts and contains no machine-specific paths.
"""
...
class DiffusionPipeline(BasicModel):
    ...
    def __init__(self,
        ...
        clip_x0=True, # Has only an effect during training if use_self_conditioning=True, important for inference/sampling
        use_ema = False,
        ema_kwargs=None,
        optimizer=torch.optim.AdamW,
        optimizer_kwargs=None, # stable-diffusion ~ 1e-4
        ...
        noise_scheduler_kwargs = dict(noise_scheduler_kwargs or {})
        noise_estimator_kwargs = dict(noise_estimator_kwargs or {})
        ema_kwargs = dict(ema_kwargs or {})
        optimizer_kwargs = dict(optimizer_kwargs or {"lr": 1e-4})
        lr_scheduler_kwargs = dict(lr_scheduler_kwargs or {})
    ...
    def _step(self, batch: dict, batch_idx: int, state: str, step: int, optimizer_idx:int):
        ...
        # print('x_0', x_0.shape)
        s = x_0.count_nonzero(dim=(1, 2, 3))  # 在 batch 维度统计
        # print(s)
        ...
        # print("xt:", x_t.shape)
        pred, pred_vertical = noise_estimator(x_t, t, s, condition, self_cond)
        ...
        # print('pred', pred.shape, 'target', target.shape, 'condition2', condition2.shape)
        # condition2 = F.interpolate(condition2, (4, 32, 32))
        # pred1 = pred * condition2
        # target1 = target * condition2

        # pred2 = pred*(1-condition2)
        # target2 = target * (1-condition2)

        # loss += self.loss_fct(pred1, target1)*weights[0] * 10000
        # loss += self.loss_fct(pred2, target2)*weights[0]
        # # ----------------- MSE/L1, ... ----------------------
        ...
        if self.global_step != 0 and self.global_step % self.sample_every_n_steps == 0:
            ...
            s_sample = s[0:self.num_samples] if s is not None else None
    ...
    def forward(self, x_t, t, s, condition=None, self_cond=None, guidance_scale=1.0, cold_diffusion=False, un_cond=None):
        ...
        if (condition is not None) and (guidance_scale != 1.0):
            # Model prediction
            pred_uncond, _ = noise_estimator(x_t, t, s, condition=un_cond, self_cond=self_cond)
            pred_cond, _ = noise_estimator(x_t, t, s, condition=condition, self_cond=self_cond)
            ...
            pred, _ =  noise_estimator(x_t, t, s, condition=condition, self_cond=self_cond)
            if self.estimate_variance:
    ...
    @torch.no_grad()
    def denoise(self, x_t, s, steps=None, condition=None, use_ddim=True, **kwargs):
        self_cond = None
        ...
        for i, t in tqdm(enumerate(reversed(timesteps_array)), total=len(timesteps_array)):
            # UNet prediction
            x_t, x_0, x_T, self_cond = self(x_t, t.expand(x_t.shape[0]), s, condition, self_cond=self_cond, **kwargs)
    ...
    @torch.no_grad()
    def sample(self, num_samples, img_size, s, condition=None, **kwargs):
        template = torch.zeros((num_samples, *img_size), device=self.device)
        x_T = self.noise_scheduler.x_final(template)
        # print('x_T',x_T.shape,'s', s.shape)
        x_0 = self.denoise(x_T, s, condition=condition, **kwargs)
        return x_0


    @torch.no_grad()
    def interpolate(self, img1, img2, i = None, s=None, condition=None, lam = 0.5, **kwargs):
        ...
        img = self.denoise(img, s, i, condition, **kwargs)
        return img
