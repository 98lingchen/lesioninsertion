...
from medical_diffusion.models.embedders import TimeEmbbeding, SizeEmbbeding
...
class UNet(nn.Module):
    ...
    def __init__(self, 
        ...
        # ------------- Size-Embedder-----------
        if size_embedder is not None:
            self.size_embedder=size_embedder(**size_embedder_kwargs)
            size_emb_dim = self.size_embedder.emb_dim
        else:
            ...
            for k in range(num_res_blocks):
                ...
                seq_list.append(
                    ConvBlock(
                        ...
                        act_name=act_name,
                        dropout=dropout,
                        emb_channels=time_emb_dim + size_emb_dim
                    )
                )
                ...
                seq_list.append(
                    Attention(
                        ...
                        norm_name=norm_name,
                        dropout=dropout,
                        emb_dim=time_emb_dim + size_emb_dim,
    ...
    def forward(self, x_t, t=None, s=None, condition=None, self_cond=None):
        # x_t [B, C, *]
        # t [B,]
        # condition [B,]
        # self_cond [B, C, *]
        ...
        # print("t:",t.shape,"time_emb:",time_emb.shape,"s:",s.shape)
        if s is None:
            size_emb = None 
        else:
            size_emb = self.size_embedder(s) # [B, C]
        # print("s:",s.shape,"size_emb:",size_emb.shape)
        ...
        if size_emb is None:
            ...
            emb = torch.cat([time_emb, size_emb], dim=1)
        # emb = torch.cat([time_emb, size_emb], dim=1)
        ...
        # x_t = torch.cat([x_t, cond_emb], dim=1)   # cond_with_tumor_mask
            # --------- Condition with tumor mask
        # print("x_t:",x_t.shape,"cond_emb:",condition.shape, 'emb', emb.shape)
        # print()
        if condition is not None:
            x_t = torch.cat([x_t, condition], dim=1)   # cond_with_tumor_mask
        # --------- Encoder --------------
        x = [self.in_conv(x_t)]
...
class UNet_nosize(nn.Module):
    ...
    def forward(self, x_t, t=None, condition=None, self_cond=None):
        ...
        # x_t = torch.cat([x_t, cond_emb], dim=1)   # cond_with_tumor_mask
            # --------- Condition with tumor mask
        # print("x_t:",x_t.shape,"cond_emb:",condition.shape)
        # print()
        if condition is not None:
            x_t = torch.cat([x_t, condition], dim=1)   # cond_with_tumor_mask
        # --------- Encoder --------------
...
class UNet_nosize_noVAE(nn.Module):
